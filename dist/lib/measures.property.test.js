...
import { shortestPath, radaSimilarity, wuPalmer, } from './measures';
const NODE_NAMES = ['n0', 'n1', 'n2', 'n3', 'n4', 'n5', 'n6', 'n7'];
const treeArb = fc.array(fc.tuple(fc.constantFrom(...NODE_NAMES), fc.constantFrom(...NODE_NAMES)), { minLength: 1, maxLength: 10 }).map(edges => {
...
describe('Property-based tests', () => {
    it('P1: shortestPath is symmetric', () => {
        fc.assert(fc.property(treeArb, ({ graph, nodes }) => {
            if (nodes.length < 2)
                return true;
            const a = nodes[0];
            const b = nodes[1];
            ...
            for (let i = 0; i < nodes.length; i++) {
                for (let j = i + 1; j < nodes.length; j++) {
                    ...
                    if (path > 0) {
                        ...
                        if (Math.abs(sim - expected) > 1e-10)
                            return false;
                    }
                }
            }
...
//# sourceMappingURL=measures.property.test.js.map
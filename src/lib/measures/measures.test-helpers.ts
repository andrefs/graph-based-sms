...
export function runMeasureTests(
  measureName: string,
  measureFn: (g: MultiDirectedGraph, c1: string, c2: string, options?: any) => number,
  options?: any
) {
  describe(measureName + ' (legacy childToParent)', () => {
    ...
    for (const tc of legacyTestCases) {
      it(`returns expected value for ${tc.name}`, () => {
        ...
        const expected = (tc.expected as any)[measureFn.name];
...
export function runMeasureTestsDefault(
  measureName: string,
  measureFn: (g: MultiDirectedGraph, c1: string, c2: string, options?: any) => number,
  options?: any
) {
